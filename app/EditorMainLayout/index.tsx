...
const items2: MenuProps['items'] = settingsMenu.map((icon, index) => {
    return {
      ...
      label: <Link href={{ pathname: `${settingsMenu[index].href}` }}>{settingsMenu[index].label}</Link>
    }
  }
)

  const defaultSelectedMenu = React.useMemo(() => {
    const defaultValue = settingsMenu.find(item => item.href === router.asPath ? `${item.key}` : null)
