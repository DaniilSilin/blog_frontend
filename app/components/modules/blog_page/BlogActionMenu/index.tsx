...
export default function BlogActionMenu({ hasAccess, blogData, slug }: Props) {
  ...
  if (Object.keys(user).length === 0) {
    return (
        <>
          <div ref={subscribeButtonRef} onClick={() => setOpenSubscribePopup(!openSubscribePopup)} className={styles.subscribeButton}>Подписаться</div>
          {openSubscribePopup && (
              <div style={{ width: '378px', height: '174px', backgroundColor: '#212121', position: 'absolute' }}>
                <div style={{ margin: '16px 0 16px 0', color: '#d2d2d2' }}>Хотите подписаться на этот канал?</div>
                <div style={{ margin: '4px 0 32px', color: '#919191' }}>Тогда войдите в аккаунт.</div>
                <Link href={`/login`}>
                  <div>Войти</div>
                </Link>
              </div>
