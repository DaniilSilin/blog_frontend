    ...
    <div style={{ display: 'flex' }}>
      <ul className={styles.ul_list}>
        <li className={styles.li_list}>
          ...
          <div style={{ fontSize: '16px' }} onMouseOver={visibleFunction} onMouseLeave={isVisibleFunction}>
                  ...
                  <div>
                    <div className={styles.liked_users_container}>
                      {post.liked_users.map((user: any) => (
                          <div key={user.id} className={styles.liked_users_container_text}>
                            <Link className={styles.liked_users_container_text}
                                  href={`/profile/${user.username}/`}>{user.username}</Link>
                          </div>
                      ))}
                      {post?.likedUsersCount > 5 ? <div>Показать ещё {post?.likedUsersCount - 5}</div> : null}</div>
                  </div>
                  )}
                </>
            ) : null}
        ...
        <li className={styles.li_list}>
          ...
          {post.commentCount ? <div style={{ fontSize: '16px' }}>{post.commentCount}</div> : null}
        </li>
        <li className={styles.li_list}>
          <Link style={{ display: 'flex' }} href={`/blog/${slug}/post/${post.post_id}/#comments`}>
