...
import { emailValidator, mapValidator, titleValidator, vkValidator, telegramValidator, youtubeValidator, dzenValidator, ownSiteLinkValidator } from '@/app/components/modules/form/validators'
...
const emailDescription = ' Укажите, как связаться с вами по вопросам сотрудничества. Зрители могут увидеть адрес электронной почты на вкладке "О канале".'

export default function BlogEdit({ slug }) {
  const { data, refetch: refetchBlog } = DjangoService.useGetBlogQuery({ slug })
  ...
  const [ initialBannerSmall, setInitialBannerSmall ] = React.useState<string | undefined>()
  const [ initialBannerOriginal, setInitialBannerOriginal ] = React.useState<string | undefined>()
  ...
  const [ initialAvatarSmall, setInitialAvatarSmall ] = React.useState<string | undefined>()
  const [ initialAvatarOriginal, setInitialAvatarOriginal ] = React.useState<string | undefined>()

  const [ initialTitle, setInitialTitle ] = React.useState<string | undefined>()
  const [ initialDescription, setInitialDescription ] = React.useState<string | undefined>()
  ...
  const [ initialVkLink, setInitialVkLink ] = React.useState<string | undefined>()
  const [ initialTelegramLink, setInitialTelegramLink ] = React.useState<string | undefined>()
  const [ initialYoutubeLink, setInitialYoutubeLink ] = React.useState<string | undefined>()
  const [ initialDzenLink, setInitialDzenLink ] = React.useState<string | undefined>()
  const [ initialOwnSiteLink, setInitialOwnSiteLink ] = React.useState<string | undefined>()
  ...
  const [ telegramLink, setTelegramLink ] = React.useState<string | undefined>(initialTelegramLink)
  const [ youtubeLink, setYoutubeLink ] = React.useState<string | undefined>(initialYoutubeLink)
  const [ dzenLink, setDzenLink ] = React.useState<string | undefined>(initialDzenLink)
  const [ ownSiteLink, setOwnSiteLink ] = React.useState<string | undefined>(initialOwnSiteLink)
  ...
  const [ youtubeLinkError, setYoutubeLinkError ] = React.useState<string | undefined>()
  const [ dzenLinkError, setDzenLinkError ] = React.useState<string | undefined>()
  const [ ownSiteLinkError, setOwnSiteLinkError ] = React.useState<string | undefined>()
  ...
  }, [ initialTitle, initialDescription, initialEmail, initialMap, initialPhone, initialVkLink, initialTelegramLink, initialYoutubeLink, initialDzenLink, initialOwnSiteLink,
    setTitle, setDescription, setEmail, setMap, setPhone, setVkLink, setTelegramLink, setYoutubeLink, setDzenLink, setOwnSiteLink,
    setIsBannerDeleted, setIsAvatarDeleted, setCroppedAvatar, setCroppedAvatarUrl, setCroppedBanner, setCroppedBannerUrl])
  ...
  const formValidator = React.useCallback(() => {
    ...
    const tg_value = validateField(telegramLink, telegramValidator, setTelegramLinkError)
    const youtube_value = validateField(youtubeLink, youtubeValidator, setYoutubeLinkError)
    const dzen_value = validateField(dzenLink, dzenValidator, setDzenLinkError)
    const own_site_value = validateField(ownSiteLink, ownSiteLinkValidator, setOwnSiteLinkError)

    return (title_value && email_value && map_value && vk_value && tg_value && youtube_value && dzen_value && own_site_value)
  }, [ title, email, map, vkLink, telegramLink, youtubeLink, dzenLink, ownSiteLink ])

  React.useEffect(() => {
    formValidator()
  }, [ title, email, map, phone, vkLink, telegramLink, youtubeLink, dzenLink, ownSiteLink ])
  ...
  }, [ isBannerDeleted, croppedBannerUrl, croppedBanner, data, initialBannerSmall ])
  ...
  }, [ isAvatarDeleted, croppedAvatarUrl, croppedAvatar, data, initialAvatarSmall ])
  ...
  React.useEffect(() => {
    if (title === initialTitle && description === initialDescription && email === initialEmail && initialMap === map && initialPhone === phone && initialVkLink === vkLink &&
      initialTelegramLink === telegramLink && initialYoutubeLink === youtubeLink && initialDzenLink === dzenLink && initialOwnSiteLink === ownSiteLink &&
        initialBannerSmall === bannerState && initialAvatarSmall === avatarState
    ) {
      setHasPostChanged(false)
    } else {
      setHasPostChanged(true)
    }
  }, [ title, description, email, map, phone, vkLink, telegramLink, youtubeLink, dzenLink, ownSiteLink,
    initialTitle, initialDescription, initialEmail, initialMap, initialPhone, initialVkLink, initialTelegramLink, initialYoutubeLink, initialDzenLink, initialOwnSiteLink,
    setHasPostChanged, initialAvatarSmall, avatarState, initialBannerSmall, bannerState ])
  ...
  return (
    <div>
            ...
            <div className={styles.bannerActionsContainer}>
              ...
              <>
                <button className={styles.bannerUploadButton}>
                  <label>
                    Изменить
                    <input type={'file'} accept='image/png,image/jpeg,image/gif' style={{ cursor: 'pointer' }} onChange={onSelectBannerImage} />
                  </label>
                </button>
                <button onClick={deleteBanner} className={styles.bannerDeleteButton}>
                  Удалить
                </button>
              </>
            )}
          ...
          <div className={styles.avatarUploadContainer}>
            ...
            <div className={styles.avatarActionsContainer}>
              ...
              <>
                ...
                <button onClick={deleteAvatar} className={styles.avatarDeleteButton}>
                  Удалить
                </button>
              </>
            )}
      ...
      <div className={styles.otherFields}>
        <UpdateInput width={400} height={40} label={'Название блога'} defaultValue={initialTitle} error={titleError} value={title} onChange={setTitle} description={titleDescription} />
        <UpdateInput width={400} height={40} defaultValue={data?.slug} label={'Псевдоним'} description={slugDescription} disabled={true} />
        <UpdateTextArea width={400} height={100} label={'Описание'} defaultValue={initialDescription} value={description} onChange={setDescription} autoSize={true} showCount={true} />
        <UpdateInput width={400} height={40} defaultValue={initialEmail} label={'Адрес электронной почты'} value={email} onChange={setEmail} error={emailError} description={emailDescription} />
        <UpdateTextArea width={400} height={100} label={'Карта'} defaultValue={initialMap} value={map} onChange={setMap} error={mapError} autoSize={true} showCount={true} />
        <PhoneInput label={'Номер телефона'} defaultValue={initialPhone} value={phone} description={'Можете добавить номер телефона. Только Российские номера!'} onChange={setPhone} />
        <div>
          <div style={{ fontSize: '22px', marginBottom: '15px' }}>Ссылки на Веб-ресурсы</div>
          <UpdateInput width={400} height={40} label={'Ссылка на ВКонтакте'} defaultValue={initialVkLink} error={vkLinkError} value={vkLink} onChange={setVkLink} />
          <UpdateInput width={400} height={40} label={'Ссылка на Telegram'} defaultValue={initialTelegramLink} error={telegramLinkError} value={telegramLink} onChange={setTelegramLink} />
          <UpdateInput width={400} height={40} label={'Ссылка на Youtube'} defaultValue={initialYoutubeLink} error={youtubeLinkError} value={telegramLink} onChange={setYoutubeLink} />
          <UpdateInput width={400} height={40} label={'Ссылка на Дзен'} defaultValue={initialDzenLink} error={dzenLinkError} value={dzenLink} onChange={setDzenLink} />
          <UpdateInput width={400} height={40} label={'Ссылка на свой Веб-ресурс'} defaultValue={initialOwnSiteLink} error={ownSiteLinkError} value={ownSiteLink} onChange={setOwnSiteLink} />
        </div>
      </div>
      <div className={styles.actionButtonsContainer}>
        ...
        <button className={classNames(styles.cancelButton, {[styles.active]: hasPostChanged })} disabled={!hasPostChanged} onClick={setToDefaultHandleChange}>
          Отмена
        </button>
        <button className={classNames(styles.submitButton, {[styles.active]: isReadyToSubmit })} disabled={!isReadyToSubmit} onClick={updateBlogData}>
          Сохранить
        </button>
      </div>
    </div>
