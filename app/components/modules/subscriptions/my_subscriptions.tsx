...
export default function MySubscriptions() {
  const { data } = DjangoService.useGetMySubscriptionsQuery()
        ...
        <div key={index}>
          {item?.subscriptions?.map((item, index) => (
            <div key={index} className={styles.subscriptionContainer}>
              ...
              <div style={{ padding: '10px', backgroundColor: 'red', width: '100px', borderRadius: '10px', cursor: 'pointer' }} onClick={() => unsubscribe( item.slug )}>
                Отписаться
              </div>
              </div>
            </div>
