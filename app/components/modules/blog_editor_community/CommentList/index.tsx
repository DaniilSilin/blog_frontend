...
export default function CommentList({ comment, slug, setSelectedBlogComments, selectedBlogComments }: Props) {
  const bodyRef = React.useRef(null)
  const user = useAppSelector(state => state.django.profile)
  const [ displayCommentInputReply, setDisplayCommentInputReply ] = React.useState(false)
  const [ displayAdditionalMenu, setDisplayAdditionalMenu ] = React.useState(false)
  ...
  const likeCommentButton = () => {
    setOrRemoveCommentLike({ slug: slug, post_id: comment.post.post_id, comment_id: comment?.comment_id })
  }

  const dislikeCommentButton = () => {
    setOrRemoveCommentDislike({ slug: slug, post_id: comment.post.post_id, comment_id: comment?.comment_id })
  }

  const deleteCommentButton = () => {
    deleteComment({ slug: slug, post_id: comment.post.post_id, comment_id: comment?.comment_id })
  }

  const setOrRemoveLikeByAuthorButton = () => {
    setOrRemoveLikeByAuthor({ slug: slug, post_id: comment.post.post_id, comment_id: comment?.comment_id })
  }
  ...
  const [ triggerQuery, { data: replyList }] = DjangoService.useLazyBlogCommentsQuery()
  const [ wasDisplayedOnce, setWasDisplayedOnce ] = React.useState(false)

  const getReplies = React.useCallback(() => {
    if (!wasDisplayedOnce) {
      triggerQuery({ slug: slug, post_id: comment.post.post_id, parent_id: comment.comment_id, page: 1 })
      setWasDisplayedOnce(true)
    }
    setDisplayReplies(!displayReplies)
  }, [ wasDisplayedOnce, setWasDisplayedOnce, setDisplayReplies, displayReplies, triggerQuery, comment, slug ])

  const uploadMoreReplies = React.useCallback(() => {
    triggerQuery({ slug: slug, post_id: comment.post.post_id, parent_id: comment.comment_id, page: page + 1 })
    setPage(page + 1)
      ...
      <div className={styles.root}>
        <div className={styles.commentAndInputReplyContainer}>
          <div style={{display: 'flex'}}>
            <div className={styles.userCommentContainer}>
              <CommentInput
                comment={comment}
                checked={selectedBlogComments.find((item: any)=> item.comment_id === comment.comment_id)}
            ...
            <div className={styles.authorAvatarContainer}>
              <Link href={`/profile/${comment.author.username}/`}>
                {comment.author.avatar_small ? (
                  <Image src={`${BASE_URL}${comment.author.avatar_small}`} className={styles.authorAvatar} alt={''} width={50} height={50} />
                ) : (
                  <Image src={'/img/default/avatar_default.jpg'} className={styles.authorAvatar} alt={''} width={50} height={50} />
                )}
              </Link>
            </div>
            <div className={styles.commentMain}>
              ...
              <div className={styles.commentActionButtons}>
                ...
                {!!comment.count_of_replies ? (
                    <button onClick={getReplies} className={styles.showMoreRepliesButton}>
                      {countOfReliesLabel}
                      {displayReplies ? <IoIosArrowUp className={styles.arrow} /> : <IoIosArrowDown className={styles.arrow} />}
                    </button>
                ) : (
                ...
                <div>
                  {comment.liked_by_author.toString() === 'true' ? (
                    <div className={classNames(styles.likedByAuthor, {[styles.active]: user.username === comment.post.author.username })} onClick={setOrRemoveLikeByAuthorButton}>
                      <Image src={comment.post.author.avatar_small ? `${BASE_URL}${comment.post.author.avatar_small}` : '/img/default/avatar_default.jpg'} className={styles.authorAvatar} alt={''} width={25} height={25} />
          ...
          <div className={styles.relatedPostContainer}>
            <div>
              <Link href={`/blog/${comment.post.blog.slug}/post/${comment.post.post_id}/`}>
                <Image src={comment.post.blog.avatar_small ? `${BASE_URL}${comment.post.blog.avatar_small}` : '/img/default/avatar_default.jpg'} className={styles.relatedPostAvatar} alt={''}
                           width={50} height={50} />
              </Link>
            </div>
            <div>
        ...
        <div>
          {displayCommentInputReply && (
            <CommentReply slug={slug} comment={comment} setDisplayCommentInputReply={setDisplayCommentInputReply}/>
          )}
        </div>
      </div>
      {displayReplies && (
        <>
          <div>
            {replyList?.results.map((commentReply: Comment, index: number) => (
              <ReplyList key={index} commentReply={commentReply} slug={slug} setSelectedBlogComments={setSelectedBlogComments} selectedBlogComments={selectedBlogComments} />
            ))}
          </div>
          {!!replyList?.next && (
