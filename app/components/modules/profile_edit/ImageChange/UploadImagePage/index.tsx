...
export default function UploadImagePage({ username }: Props) {
    ...
    reader.addEventListener("load", () => {
      ...
      imageElement.addEventListener('load', (e) => {
        ...
        if (height < MIN_DIMENSION || width < MIN_DIMENSION) {
          setErrorMessage('Фотография должна иметь размер не менее 200 точек в ширину и не менее 200 точек в высоту.')
  ...
  if (imageSourceUrl) {
    return (
      <CropSourceImage imageSourceUrl={imageSourceUrl} username={username} sourceImage={sourceImage} />
    )
  }
  ...
  return (
    <div className={styles.root}>
      ...
      <div className={styles.ownerAvatarEditBody}>
        ...
        <div className={styles.ownerAvatarEditBodyTitle}>Друзьям будет проще узнать вас, если вы загрузите свою настоящую фотографию.
          Вы можете загрузить изображение в формате JPG, GIF, PNG, WEBP или HEIC/HEIF.
        </div>
        <div className={styles.ownerAvatarEditorButton}>
          <input type={'file'} accept='image/png,image/jpeg,image/gif,image/heic,image/heif,image/webp' onChange={onSelectFile} />
        </div>
      </div>
      <div className={styles.ownerAvatarEditFooter}>Если у вас возникают проблемы с загрузкой, попробуйте выбрать фотографию меньшего размера.</div>
    </div>
  )
}
