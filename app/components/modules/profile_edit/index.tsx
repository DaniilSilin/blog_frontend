...
import { emailValidator, firstNameValidator, lastNameValidator, mapValidator } from '@/app/components/modules/form/validators'
...
const slugDescription = 'Ваше имя пользователя. Менять имя пользователя нельзя.'
...
export default function  ProfileEdit({ username }) {
  ...
  const [ initialFirstName, setInitialFirstName ] = React.useState<string | undefined>()
  const [ initialLastName, setInitialLastName ] = React.useState<string | undefined>()
  ...
  const [ initialGender, setInitialGender ] = React.useState<string | undefined>()
  ...
  }, [ initialFirstName, initialLastName, initialDescription, initialEmail, initialMap, initialGender, setInitialFirstName, setInitialLastName, setDescription,
    setEmail, setMap, setGender, setIsBannerDeleted, setIsAvatarDeleted, setCroppedAvatar, setCroppedAvatarUrl, setCroppedBanner, setCroppedBannerUrl ])
  ...
  React.useEffect(() => {
    if (title === initialTitle && description === initialDescription && email === initialEmail && initialBannerSmall === bannerState &&
    ...
    initialTitle, initialDescription, initialEmail, initialMap,
  ...
  const userGenderHandleChange = React.useCallback((value: string) => {
    setGender(value)
  }, [ setGender ])
  ...
  return (
    <div>
      ...
      <div className={styles.otherFields}>
        <UpdateInput width={400} height={40} label={'Имя'} defaultValue={initialFirstName} error={firstNameError} value={firstName} onChange={setFirstName} />
        <UpdateInput width={400} height={40} label={'Фамилия'} defaultValue={initialLastName} error={lastNameError} value={lastName} onChange={setLastName} />
        <UpdateInput width={400} height={40} label={'Имя пользователя'} defaultValue={data.username} description={slugDescription} disabled={true} />
        ...
        <Select data={genderList} defaultValue={initialGender} onChange={userGenderHandleChange} gender={gender} label={'Пол'} />
      </div>
