...
export default function BlogEditorPublications({ slug }) {
  ...
  const { data } = DjangoService.useBlogEditorPostsQuery({ slug: slug, state: state, sortOrder: sortOrder, columnType: columnType })
  ...
  const postListHandleChange = React.useCallback((item: any) => {
  ...
  const handleChangeCheckbox = React.useCallback((checked: boolean, post: any) => {
    if (checked) {
      setSelectedPosts([...selectedPosts, post])
    } else {
      setSelectedPosts(selectedPosts.filter(item => item.post_id !== post.post_id))
    }
    ...
    <div className={styles.root}>
      ...
      <div className={styles.tabsMenu}>
        {POST_LIST_MENU.map((item, index) => (
          <div key={index} className={classNames(styles.postTabsMenu, {[styles.active]: `${router.pathname}?state=${state}` === item.pathname })}>
            <div onClick={() => postListHandleChange(item)}>{item.title}</div>
            {item.id === 1 ? <div style={{ marginLeft: '10px' }}>{data?.count_of_published_posts}</div> : null}
            {item.id === 2 ? <div style={{ marginLeft: '10px' }}>{data?.const_of_drafted_posts}</div> : null}
          </div>
        ))}
      </div>
      ...
      <div>
        ...
        <thead>
            <tr>
              <th></th>
              <th>Публикация</th>
              <th className={classNames(styles.dateTh, {[styles.active]: columnType === 'date' })} onClick={dateColumnHandleChange}>
                {columnType === 'date' ? (
                  <>
                    Дата
                    {sortOrder === 'descending' && <FaArrowUpLong className={styles.arrow} />}
                    {sortOrder === 'ascending' && <FaArrowDownLong className={styles.arrow} />}
                  </>
                ) : (
                  <>
                    Дата
              ...
              <th className={classNames(styles.viewsTh, {[styles.active]: columnType === 'views' })} onClick={viewsColumnHandleChange}>
                {columnType === 'views' ? (
                  <>
                    Просмотры
                    {sortOrder === 'descending' && <FaArrowUpLong className={styles.arrow} />}
              ...
              <th className={classNames(styles.commentsTh, {[styles.active]: columnType === 'comments' })} onClick={commentsColumnHandleChange}>
                {columnType === 'comments' ? (
                  <>
                    Просмотры
                    {sortOrder === 'descending' && <FaArrowUpLong className={styles.arrow} />}
          ...
          <tbody>
              {data?.results.map((post) => (
                <tr>
                  <td>
                    <PostInputCheckbox onChange={handleChangeCheckbox} checked={selectedPosts.find((item) => item.post_id === post.post_id)} post={post} />
                  </td>
                  <td>
